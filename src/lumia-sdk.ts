...
export default class LumiaSdk extends EventEmitter {
  ...
  private _startWebsockets = () => {
    return new Promise((resolve, reject) => {
      ...
      this._websocket = new Sockette(
        `ws://${this._data.host}:${this._data.port}/api?token=${this._data.token}&name=${this._data.name}`,
        {
          timeout: 5000,
  ...
  sendGamesGlowVariableUpdate = async (pack: {
    gamesGlowKey: string;
    value: any;
  }) => {
    return this.send({
  ...
  sendGamesGlowVirtualLightsChange = async (pack: {
    gamesGlowKey: string;
    value: any;
  }) => {
    return this.send({
